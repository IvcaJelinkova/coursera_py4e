...
row = cur.fetchone()
if row is not None:
    print('''Restarting existing crawl. Remove spider.sqlite to start
        a fresh crawl. ''')
else:
    ...
    if (len(web) > 1):
        cur.execute('''INSERT OR IGNORE INTO Webs (url) 
            VALUES (?)''', (web,))  ###
...
while True:
    ...
    try:
        ...
        if 'text/html' != document.info().get_content_type():
            ...
            cur.execute('UPDATE Pages SET error = 0 WHERE url = ?',
                        (url, ))
            conn.commit()
            continue
